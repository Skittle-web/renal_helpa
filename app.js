...
class RentalApp {
    constructor() {
        ...
        // Убираем проверку на Electron, оставляем только браузерный режим
        this.isElectron = false;
        this.data = { properties: [], golfCarts: [], bookings: [] };
        ...
        // Translations (оставляем без изменений)
        this.translations = {
            ru: {
        ...
        this.userId = null; // ID текущего пользователя
    ...
    async loadFromFirestore() {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                console.log('Данные загружены для пользователя:', this.userId);
            } else {
    ...
    async saveToFirestore() {
        ...
        try {
            ...
            console.log('Данные сохранены для пользователя:', this.userId);
            return true;
        } catch (error) {
    ...
    // Заглушки для старых методов (удаляем их)
    loadDataFromElectron() { return { properties: [], golfCarts: [], bookings: [] }; }
    loadLanguageFromElectron() { return 'ru'; }
    loadFromLocalStorage() { return { properties: [], golfCarts: [], bookings: [] }; }
    saveData() { return false; } // больше не используем
    ...
    deleteBookingFromModal(bookingId, modalIdToClose) {
        if (confirm(this.t('deleteBookingConfirm'))) {
            ...
            const success = this.saveToFirestore();
            if (success) {
                this.refreshAllData();
                this.closeModal(modalIdToClose);
            } else {
    ...
    async init() {
        ...
        this.setupImportantClickHandler();
        this.setupAuth(); // НОВОЕ: настраиваем авторизацию
        this.updateUI();
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            // Показываем панель для авторизованных
            ...
            // Показываем панель для неавторизованных
...
async logout() {
    ...
    setupEventListeners() {
        ...
        // Сохранение (теперь все async)
        document.getElementById('save-property')?.addEventListener('click', async (e) => {
            e.preventDefault();
            await this.saveProperty();
        });

        const saveGolfBtn = document.getElementById('save-golf-cart');
        if (saveGolfBtn) {
            saveGolfBtn.addEventListener('click', async (e) => {
                e.preventDefault();
                await this.saveGolfCart();
            });
        }

        document.getElementById('save-booking')?.addEventListener('click', async (e) => {
            e.preventDefault();
    ...
    showImportantMessage(title, message) {
        console.log('Showing important message:', title, message);
    ...
    async saveProperty() {
        ...
        const success = await this.saveToFirestore();
        if (success) {
            this.refreshAllData();
            this.hideModal('property-modal');
        } else {
    ...
    async saveGolfCart() {
        console.log('💾 saveGolfCart начал работу');
        
        const nameInput = document.getElementById('golf-cart-name');
        const ownerInput = document.getElementById('golf-cart-owner');
        ...
        const color = document.getElementById('golf-cart-color')?.value || '#FF9800';
        const important = document.getElementById('golf-cart-important')?.checked || false;
        const importantText = document.getElementById('golf-cart-important-text')?.value.trim() || '';

        console.log('Данные:', { name, owner, seats, description, color, important });
        ...
        if (this.currentEditingGolfCart) {
            ...
            console.log('Обновлена существующая машина:', this.currentEditingGolfCart);
        } else {
            const newCart = {
                id: Date.now(),
                name,
            ...
            console.log('Создана новая машина:', newCart);
        }

        console.log('Сохраняем данные...');
        const success = await this.saveToFirestore();
        console.log('saveToFirestore success:', success);
